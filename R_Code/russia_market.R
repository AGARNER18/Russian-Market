...
#1-30473
train<-read.csv("train_without_noise.csv")
#30474-38135
...
c$cafe_sum_500_min_price_avg<-NULLL
c$female_f<-NULL
c$full_all<-NULL
...
error<-sqrt(msle(a$price_doc, (predict(model_no_outliers_grown, a))))
error
...
error_tuned<-msle(a$price_doc, (predict(model2, a)))
...
pred <- predict(model_no_outliers_grown, b)

predictions <- data.frame(id=b$id, price_doc=pred)

write.csv(predictions, "rforestrussia_no_outliers.csv")
